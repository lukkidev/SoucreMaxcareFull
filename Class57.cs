using System;
using System.Collections.Generic;
using System.IO;
using OpenQA.Selenium.Chrome;

internal class Class57
{
	public static string string_0 = smethod_0();

	public static Random random_0 = new Random();

	public static List<string> list_0 = new List<string> { "app.minsoftware.vn", "app.minsoftware.xyz", "beta.minsoftware.vn" };

	public static string string_1 = GClass8.A2307E3B("");

	public static string A0283E24 = GClass8.A2307E3B("");

	public static bool bool_0 = false;

	public static bool DE24BEB7 = false;

	public static bool bool_1 = false;

	public static bool bool_2 = false;

	public static bool bool_3 = false;

	public static bool bool_4 = false;

	public static bool bool_5 = false;

	public static bool bool_6 = false;

	public static bool bool_7 = false;

	public static int int_0 = -2;

	public static F7831EB3 A0BD9AA4;

	public static GClass17 gclass17_0;

	public static ChromeDriverService D82A9313;

	public static object object_0 = new object();

	public static object FB95E624 = new object();

	public static object object_1 = new object();

	public static List<string> list_1 = new List<string> { "Phục vụ tận tình, vui vẻ. Nói chung là tuyệt\ufffd\ufffd" };

	private static object object_2 = new object();

	private static bool bool_8 = false;

	public static string smethod_0()
	{
		if (GClass25.smethod_0("configGeneral").method_3("ckbDongBoMaxCare") && Directory.Exists(GClass25.smethod_0("configGeneral").C0288288("txtPathMaxCare")))
		{
			return GClass25.smethod_0("configGeneral").C0288288("txtPathMaxCare");
		}
		return GClass28.smethod_0();
	}

	public static string E0AE7DB2()
	{
		return GClass25.smethod_0("settingLogin").C0288288("Server", "1");
	}

	public static Dictionary<string, string> smethod_1()
	{
		return new Dictionary<string, string>
		{
			{ "1", "VietNam" },
			{ "2", "English" },
			{ "3", "Cambodia" },
			{ "4", "Thailand" }
		};
	}

	public static string smethod_2()
	{
		string text = GClass25.smethod_0("configGeneral").C0288288("Language");
		if (text == "")
		{
			text = ((!(E0AE7DB2() == "1")) ? "EN" : "VN");
		}
		return text;
	}

	public static void EC2BD382(string string_2 = "VN")
	{
		GClass25.smethod_0("configGeneral").method_4("Language", string_2);
		GClass25.B3B73732("configGeneral");
	}

	public static bool smethod_3()
	{
		return true;
	}

	public static string smethod_4()
	{
		string text = "";
		lock (object_2)
		{
			text = list_1.smethod_6();
			list_1.Remove(text);
		}
		return text;
	}

	public static bool smethod_5()
	{
		return bool_8;
	}

	public static void A39EDD1D(bool bool_9)
	{
		bool_8 = bool_9;
	}
}
